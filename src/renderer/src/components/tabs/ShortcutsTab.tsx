      ...
      <CardContent>
        <div className="space-y-4">
          {shortcuts.map(shortcut => {
            const failed = failedShortcuts.includes(
              shortcut.key.replace('Ctrl', 'CommandOrControl')
            )
            ...
            return (
              ...
              >
                ...
                <kbd className="px-2 py-1 text-sm font-mono bg-muted rounded">{shortcut.key}</kbd>
              </div>
            )
          })}
        </div>
        ...
        {settings.sessionType === 'wayland' && (
          <div className="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-md border border-yellow-200 dark:border-yellow-900">
            <p className="text-sm">
              <strong>Wayland detected:</strong> Global shortcuts may require permission dialogs or
        ...
        {failedShortcuts.length > 0 && (
          <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-md border border-red-200 dark:border-red-900">
            <p className="text-sm">
              <strong>Some shortcuts failed to register.</strong> This may be due to conflicts with
              existing shortcuts or system restrictions.
            </p>
          </div>
        )}
      </CardContent>
